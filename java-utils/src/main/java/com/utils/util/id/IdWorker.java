...
public class IdWorker {
    ...
    private final static ConcurrentSkipListSet<Long> listSet = new ConcurrentSkipListSet<Long>();//缓存一段时间内的ID防止冲突
    private final static int resetSize = 128;//初始容量
    private static int realSize = 0;// 实际容量

    private final long workerId;
    private final long snsEpoch = 1577808000000L;// 起始标记点，作为基准
    private long sequence = 0L;// 0，并发控制
    private final long workerIdBits = 4L;// 只允许workid的范围为：0-1023
    private final long maxWorkerId = -1L ^ -1L << this.workerIdBits;// 1023,1111111111,10位
    private final long sequenceBits = 6L;// sequence值控制在0-4095
    ...
    public IdWorker(){
        super();
        this.workerId = ThreadLocalRandom.current().nextLong(0, 999);
    }

    public long nextId() throws Exception {
        long gKey = nextKey();
        while (listSet.contains(gKey)) {
            gKey = nextKey();
        }
        addSet(gKey);
        return gKey;
    ...
    private synchronized long nextKey() throws Exception {
     ...
     * @param id
     */
    private void addSet(long id) {
        if (resetSize <= realSize) {
            listSet.pollLast();
        } else {
            realSize++;
        }
